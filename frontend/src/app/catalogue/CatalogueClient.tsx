...
// ─── Props pour les données SSR initiales ──────────────────
interface CatalogueClientProps {
  initialData?: (Product & { category_slug?: string; category_name?: string })[];
...
// ─── Page Catalogue ────────────────────────────────────────
export default function CatalogueClient({ initialData, initialCategories }: CatalogueClientProps) {
  const [allProducts, setAllProducts] = useState<(Product & { category_slug?: string; category_name?: string })[]>(
    initialData ?? []
  );
  const [categories, setCategories] = useState<CategoryFilter[]>(initialCategories ?? []);
  const [activeCategory, setActiveCategory] = useState<string>("all");
  const [loading, setLoading] = useState(!initialData || !initialCategories);

  useEffect(() => {
    // If SSR provided data, we just use it as-is — no client fetch needed on mount.
